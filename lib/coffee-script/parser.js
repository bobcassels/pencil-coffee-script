...
case 208:
this.$ = yy.addLocationDataFn(_$[$0-2], _$[$0])((function () {
        var aDen, aMatch, aNum, bDen, bMatch, bNum, gcd, rDen, rNum, ratio, rem, sign, _ref;
        if ($$[$0-1] === '/' && $$[$0-2] instanceof yy.Value && $$[$0-2].bareLiteral(yy.Literal) && $$[$0-2].base.kind === 'NUMBER' && /^[-+]?\d+(\/\d+)?$/.test($$[$0-2].base.value) && $$[$0] instanceof yy.Value && $$[$0].bareLiteral(yy.Literal) && $$[$0].base.kind === 'NUMBER' && /^[-+]?\d+(\/\d+)?$/.test($$[$0].base.value)) {
          aMatch = /^([-+])?(\d+)(\/(\d+))?$/.exec($$[$0-2].base.value);
          bMatch = /^([-+])?(\d+)(\/(\d+))?$/.exec($$[$0].base.value);
          sign = (aMatch[1] || '+') === (bMatch[1] || '+') ? '' : '-';
          aNum = +aMatch[2];
          aDen = +(aMatch[4] || 1);
          bNum = +bMatch[2];
          bDen = +(bMatch[4] || 1);
          rNum = aNum * bDen;
          rDen = aDen * bNum;
          if (rNum < Math.pow(2, 53) && (0 < rDen && rDen < Math.pow(2, 53))) {
            gcd = rNum;
            rem = rDen;
            while (rem !== 0) {
              _ref = [rem, gcd % rem], gcd = _ref[0], rem = _ref[1];
            }
            rNum = rNum / gcd;
            rDen = rDen / gcd;
            ratio = rDen === 1 ? rNum : "" + rNum + "/" + rDen;
            return new yy.Value(new yy.Literal("" + sign + ratio, 'NUMBER'));
          } else {
