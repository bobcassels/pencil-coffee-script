...
(function() {
  ...
  grammar = {
    ...
    Parenthetical: [
      o('( Body )', function() {
        if ($2 instanceof Block && $2.expressions.length === 1 && $2.expressions[0] instanceof Value && $2.expressions[0].bareLiteral(Literal) && $2.expressions[0].base.kind === 'NUMBER') {
          return $2.expressions[0];
        } else {
          return new Parens($2);
        }
      }), o('( INDENT Body OUTDENT )', function() {
        if ($3 instanceof Block && $3.expressions.length === 1 && $3.expressions[0] instanceof Value && $3.expressions[0].bareLiteral(Literal) && $3.expressions[0].base.kind === 'NUMBER') {
          return $3.expressions[0];
        } else {
          return new Parens($3);
        }
      })
    ...
    Operation: [
      o('UNARY Expression', function() {
        ...
        var negated, value;
        if ($2 instanceof Value && $2.bareLiteral(Literal) && $2.base.kind === 'NUMBER') {
          value = $2.base.value;
          if (/^\+/.test(value)) {
            value = value.slice(1);
          }
          negated = /^-/.test(value) ? value.slice(1) : "-" + value;
          return new Value(new Literal(negated, 'NUMBER'));
        } else {
          return new Op('-', $2);
        }
      }), {
        ...
        if ($2 === '/' && $1 instanceof Value && $1.bareLiteral(Literal) && $1.base.kind === 'NUMBER' && RATIONAL.test($1.base.value) && $3 instanceof Value && $3.bareLiteral(Literal) && $3.base.kind === 'NUMBER' && RATIONAL.test($3.base.value)) {
          _ref = rationalParts($1.base.value), aPositive = _ref[0], aNum = _ref[1], aDen = _ref[2], aImag = _ref[3];
          _ref1 = rationalParts($3.base.value), bPositive = _ref1[0], bNum = _ref1[1], bDen = _ref1[2], bImag = _ref1[3];
          ...
          sign = positive ? '' : '-';
          if (rational) {
            return new Value(new Literal("" + sign + rational + imag, 'NUMBER'));
          } else {
            return new Op($2, $1, $3);
          }
        } else {
